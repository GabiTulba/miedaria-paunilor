...
use crate::models::{AdminUser, NewAdminUser, User, NewUser};
use crate::schema::*;
use crate::utils;
...
pub fn create_admin(conn: &mut PgConnection, username: &str, password: &str) -> QueryResult<AdminUser> {
    let (new_salt, hash) = create_salt_and_hash(&password);

    let new_user = NewAdminUser { username:&username, salt:&new_salt, hashed_password:&hash };
...
pub fn get_admin(conn: &mut PgConnection, user: &str) -> QueryResult<Option<AdminUser>> {
    ...
    admin_users.find(user)
    .select(AdminUser::as_select())
    .first(conn)
    .optional()
}

pub fn update_admin_password(conn: &mut PgConnection, user: &str, new_password: &str) -> QueryResult<()> {
    match get_admin(conn, user)? {
        Some (_user) => {
            ...
            .set((admin_users::salt.eq(new_salt), admin_users::hashed_password.eq(hash)))
            .execute(conn)
            .map(|_| ())
        }
...
pub fn create_regular(conn: &mut PgConnection, username: &str, password: &str) -> QueryResult<User> {
    let (new_salt, hash) = create_salt_and_hash(&password);

    let new_user = NewUser { username:&username, salt:&new_salt, hashed_password:&hash };
...
pub fn update_regular_password(conn: &mut PgConnection, user: &str, new_password: &str) -> QueryResult<()> {
